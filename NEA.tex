...
\textcolor{red}{Add haversine formula here}
...
\begin{figure}
    \begin{center}
\begin{tikzpicture}
\filldraw (0,0) circle (2pt) node[anchor=south] {$p$};
\filldraw (2,2) circle (2pt) node[anchor=south] {$q$};n    
\filldraw (-2,0) circle (2pt) node[anchor=south] {$s$};
\filldraw (-2,-2) circle (2pt) node[anchor=east] {$t$};
\draw (-2,0) -- (-2,-2) node[pos=0.5,left] {$2$};
\draw (-2,-2) -- (0,-2) node[pos=0.5,below] {$3$};
\draw (-2,0) -- (0,0) node[pos=0.5,above] {$5$};
