...
\titlespacing{\chapter}{0.0ex}{0.0ex}{2.0ex}
...
\lstdefinelanguage{Kotlin}{
  ...
  emph={filter, first, firstOrNull, @Test, forEach, lazy, map, mapNotNull, println},
...
\end{center}
    ...
    \label{alternatives}
\end{figure}
There exist similar systems for route finding such as Google Maps and Citymapper. These have advantages over what I will be able to offer such as being able to use live traffic data, and having a good user interface. Citymapper has a option to choose between "Quiet", "Regular", and "Fast" routes, which seems to be based on the type of road you are taken down.
...
\begin{figure}[p]
    \begin{center}
        \includegraphics[width=15cm]{Data Flow.png}
    \end{center}
...
A Priority Queue is a useful Queue-like structure, which allows fast addition of weighted objects, and very fast retrieval of the smallest item. I used these in Dijkstra searches, and contracting graphs to find the minimum item.
\subsection{Stack and Queue}
A stack is a First in Last Out Datastructure, and a Queue is a First in First Out Datastructure.
Stacks and queues can be used for many algorithms, including iterative versions of algorithms which would traditionally be recursive.
\newpage
...
\begin{longtable}[c]{|c|p{6cm}|c|c|c|}
    ...
    \hline
    \endhead
    \ref{downloadaccidents}  & Run the \texttt{download.py} script and check that accidents are properly stored. & Normal & Figure \ref{downloadevidence} & \checkmark\\
    \hline
    \ref{parseaccidents} \& \ref{storeaccidents} & Run \texttt{parse.py} and verify that the \texttt{output.json} file is created correctly using \texttt{display.py} & Normal & Figure \ref{parsevidence}& \checkmark\\
    \hline
    \ref{item-one} \& \ref{item-two} \& \ref{item-three} \& \ref{item-four} & Run the program, and select a \texttt{.osm} file to parse, and verify it is parsed correctly by finding a route between two points. & Normal & Figure \ref{graphparsevidence} & \checkmark\\
    \hline
    ...
    \hline
    \ref{item-eight} & Attempt to find a route to a point on a motorway, and verify that the route does not actually go there & Illegal & Figure \ref{nomotorway} & \checkmark\\
    \hline
    \ref{item-ten} & Load a \texttt{.osm} file, contract the graph, then execute and check a \texttt{findRoute} query on that graph. & Normal & Figure \ref{contracttestevidence}& \checkmark\\
    \hline
    \ref{item-eleven} & Run a contracted and non contracted query on the same graph, and verify that they overlap. & Normal & Figure \ref{contractoverlap} & \checkmark\\
    \hline
    \ref{item-thirteen} \& \ref{item-fourteen} & Attempt to cause a crash by inputting illegal inputs. Verify that this fails & Illegal & Figure \ref{No illegal inputs} & \checkmark\\
    \hline
    \ref{item-sixteen} & Create and then save a map. Load the map back into memory and verify that route finding on it still works & Normal & Figure \ref{savemapevidence}& \checkmark
\end{longtable}
}
\section{Performance Testing}
\label{perf}
Performance testing is needed to test specification point \ref{item-nine}. I added a test option to the user interface that runs 1000 searches between east london and heathrow airport. As these nodes are very far apart the time taken to find routes between them should be at its maximum.
So long as this number is low enough, we can assume that all other routes would also be findable in a sufficient time period. I decided to test all algorithms with a distance cost of 15.0 and a turn cost of 0.0. 
...
\begin{longtable}[c]{|c|p{3cm}|p{5cm}|p{5cm}|}
    ...
    \hline
    \multicolumn{4}{|c|}{Continuation of Evalutation}\\
    \hline
    Specification Point & Specification & Reached? & Can it be improved?\\
    \hline
    \endhead
   
    \ref{downloadaccidents} & The System must be capable of interfacing with the TFL API to download accidents. & Yes, the Accident Download System is capable of downloading accidents from the API, as outlined in testing. & Not really as the Accident Download System only needs to be run once, so any improvements in terms of speed or usability would not make any difference to users of the main program. \\
    \hline
    \ref{parseaccidents} & The System must be capable of parsing accident data to determine which accidents are relevant. & Yes, the accident data was successfully parsed for those accidents which were relevant, as outlined in testing. & See above\\
    \hline
    \ref{storeaccidents} & The System must be capable of storing accident data in a easily accessible file for the route finding algorithm to parse. & Yes, the file \texttt{output.json} was successfully created in a readable format, as outlined in testing. & See above\\
    \hline
    \ref{item-one} & The System should be capable of processing an Open Street Map map file into a suitable data structure & The \texttt{OpenStreetMap} class contains functional methods for the parsing of the OSM file, and the \texttt{GeographicGraph} class provides a suitable data structure. This is outlined in testing. & The parsing is fairly slow (on the order of tens of seconds for large files). Parsing the Protocolbuffer Binary Format\cite{google} alternative maps could be faster, as it is easier to multithread reading from these files.\\
    \hline
    \ref{item-two} & The System should be capable of processing the accident data from the Accident Download System & The \texttt{gatherWeights} method successfully reads the data from \texttt{output.json}, as outlined in testing. & Reading \texttt{output.json} does not take that long, so a speedup would not be useful. \\
    \hline
    \ref{item-three} & The System should be capable of attaching the data from the Accident Download System to the suitable data structure from specification point \ref{item-one} & \texttt{gatherWeights} fulfills this functionality, as outlined in testing. & \texttt{gatherWeights} already uses an R\*Tree to speedup nearest weight lookup, so a speedup would be difficult, and other goals are achieved as well.\\
    \hline
    \ref{item-four} & The System should be capable of using Accident Data combined with other suitable cost estimation functions to find a directed route between two points that are places on roads in London & \texttt{findRoute} fulfills this functionality, as outlined in testing & \texttt{findRoute} could be optimised to go faster, but graph contraction already exists for this purpose. \\
    \hline
    ...
    \hline
    \ref{item-eleven} & When using the same parameters, the route generated using a preprocessed graph and associated algorithms should be the same as that generated by the non-preprocessing based approach. & As outlined in the design section, \texttt{ContractableGraph} cannot be used to calculate routes that contain turn costs. Therefore the contractable graph will not be used to generate a route when the turn cost is not set to 0, but when the turn cost is the route is the same. This is outlined in testing. & Contracting an edge-based graph of connections between edges would allow for contracted graphs to contain turn costs. \\
    \hline
    \ref{item-twelve} & The System should be easy to use & I asked my client if they found the system easy to use and they said yes. As shown in Appendix \ref{Real World Testing}, I managed to use the system to easily generate an easy to follow route. The system gives appropriate errors when illegal input is provided. & Adding a GUI to the program would make it easier to input coordinates and check that routes are appropriate.\\
    \hline
    \ref{item-thirteen} & The System should not crash, and should give appropriate non crashing errors if user input data is determined to be bad & \texttt{Main.kt} uses defensive programming and exception handling to catch errors caused by illegal user input. If there is an error, an appropriate error is given. This is detailed in testing & To the best of my knowledge all errors that would be caused by a non-malicious user have been accounted for. Problems with malicious users still remain. For example it  would be possible for a malicious user to replace files where objects are serialised with graph repesentations with multiple strongly connected components. This could be dealt with by checking objects for basic properties on deserialisation. This would not be infallible, and probably pointless as malicious users would only be malicious against themselves.\\
    \hline
    \ref{item-fourteen} & The parameters for the cost estimation functions should be user definable, but sensible defaults should also be defined & Users can input a number for their own parameter, or d for a default input. & \\
    \hline
    \ref{item-fifteen} & The Route should be output in a format that is easy for the user to follow on a mobile device & As outlined in Appendix \ref{Real World Testing}, I managed to follow a route generated by my program by importing it into Osmand and using Osmand's turn by turn instructions. & Osmand did take a while to match my route up to it's internal OSM, but I do not think there is much that can be done about this short of writing my own mobile route finding app.\\
    \hline
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=8cm]{nomotorway.png}
    \end{center}
